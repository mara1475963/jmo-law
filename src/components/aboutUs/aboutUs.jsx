...
const onClickHandler = (id)=>{
  ...
  return (
    <>
    <div className='profiles-container' id='section--2'>
        {profiles.map(({name,imgProfile,description},idx)=> {

            const [firstname,lastname] = name.split(' ')
            return(        
            <div className='profile-container' key={idx} onClick={()=>onClickHandler(idx)}>
              <h3>{firstname} <span style={{color:'red'}}>{lastname}</span></h3>
              <img src={imgProfile} />
              <div>{description}</div>
              <button onClick={()=>onClickHandler(idx)}>Vice...</button>
